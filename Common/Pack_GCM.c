...
*/
void incr_ctr(uint8_t *ctr, int size) {
    int i;
    for(i = 0; i < size; i++) {
        if ( ++ctr[i] != 0)
            return;
    ...
    if(ctr[i-1] == 0) {
        //printf("[%d] counter overflow, please implement session restart or start with a smaller counter/IV\n", __LINE__);while(1);
    }
}
...
void pack_gcm(uint8_t *dst, uint8_t *src, int size, struct ctx_gcm_s *ctx, uint8_t *mac) {
    ...
    if (sgx_ret != SGX_SUCCESS) {
        printf("Error while encrypting = %x\n", sgx_ret);
        while(1);
    }
...
void unpack_gcm(uint8_t *dst, uint8_t *src, int size, struct ctx_gcm_s *ctx, uint8_t *mac) {
    ...
    if (sgx_ret != SGX_SUCCESS) {
        printf("Error while decrypting = %x\n", sgx_ret);
        while(1);
    }
...
void alloc_gcm(struct ctx_gcm_s **ctx) {
    *ctx = (struct ctx_gcm_s*) malloc(sizeof(struct ctx_gcm_s));
    if(*ctx == NULL) {
